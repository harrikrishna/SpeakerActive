    ...
    struct AboutHeaderView: View {
        ...
        var body: some View {
            ...
            VStack (alignment: .leading) {
                ...
                Text("SpeakerActive version \(Bundle.main.versionNumber) © Harri Lyons 2024")
                    .font(.footnote)
                    .fontWeight(.light)
                ...
                if globalThings.updatingApp == false {
                    Text("A menu bar app to keep your speakers awake. Prevents power-saving switch-offs by playing an inaudible, low-frequency tone at regular intervals until the sleep timer runs out.")
                        .font(.footnote)
                        .fontWeight(.light)
                        .padding(.bottom, 6)
            ...
            VStack (alignment: .leading) {
                DisclosureGroup("Options"){
                    ScrollView (showsIndicators: false) {
                        ...
                        GroupBox {
                            VStack(alignment: .leading){
                                Toggle(isOn: $globalThings.disableSleepTimer) {
                                    Text("Disable sleep timer")
                                        .fontWeight(.bold)
                                        .font(.system(size: 12))
                                }
                                .toggleStyle(CheckboxStyle())
                                HStack{
                                    Text("Tone will play indefinitely until it's switched off manually (experimental - see Troubleshooting")
                                        .font(.footnote)
                                        .fontWeight(.light)
                                        .padding(.leading, 20)
                        ...
                        GroupBox {
                            VStack(alignment: .leading){
                                Toggle(isOn: $globalThings.runScreenOff) {
                                    Text("Keep running when system sleeps")
                                        .fontWeight(.bold)
                                        .font(.system(size: 12))
                                }
                                .toggleStyle(CheckboxStyle())
                                HStack{
                                    Text("Keeps tone playing when system idle/screen is asleep (hopefully anyway - see Troubleshooting).")
                                        .font(.footnote)
                                        .fontWeight(.light)
                                        .padding(.leading, 20)
    ...
    struct TroubleshootingView: View {
        var body: some View {
            ...
            DisclosureGroup("Troubleshooting") {
                HStack {
                    VStack (alignment: .leading) {
                        Text("**Volume issues:** The tone should be inaudible at such a low frequency, but if your ears are superhuman or the speakers pop loudly when they re-activate, try reducing the volume in the Options above. \n\nLikewise if your speakers aren't waking up, try cranking it up a little (and adjusting your system volumes - you'll need some level of general system audio for it to work, the amount depending on your individual speakers' power-saving threshold.)")
                            .font(.footnote)
                            .fontWeight(.light)
                            .padding (.top, 6)
                            .padding (.bottom, 3)
                        Text("**Sleep mode issues:** If, like mine, your speakers are connected to more things than just your Mac then you'll probably want the tone to run while the screen is off, or disable the sleep mode altogether. \n\nI've added options for these which seem to work fine for me but haven't been tested on other setups. If the tone isn't running while your system sleeps, the app may need to be used alongside a screen-waking app like Caffeine or Lungo.")
                            .font(.footnote)
                            .fontWeight(.light)
                            .padding (.top, 3)
                            .padding (.bottom, 6)
